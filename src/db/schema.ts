...
export const questions = pgTable(
  "questions",
  {
    ...
    /**
     * The examiner asking THIS question aloud, as `s3://bucket/<key>`.
     *
     * Parts 1 and 3 are an interview: on test day the question is heard, never
     * read, so the mock plays it with the text hidden. Part 2 carries the
     * topic line only — its cue card stays on screen throughout, exactly as
     * the printed card does in the real test.
     *
     * Per-question rather than per-set (`question_sets.audioUrl` is one
     * recording for a whole listening part) because each item is its own clip.
     * Generated by `npm run db:build:speaking-audio`; served through
     * src/lib/protected-media.ts, never as a public link.
     */
    promptAudioUrl: text(),
