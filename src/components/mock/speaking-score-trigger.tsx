"use client";

import { useEffect, useState, useRef } from "react";
import { useRouter } from "next/navigation";
import { Loader2, Sparkles } from "lucide-react";
import { scoreMockSpeaking, scoreMockWriting } from "@/app/actions/mock";

/**
 * Kicks off AI scoring (Writing + Speaking) for a finished mock, then refreshes
 * the report.
 *
 * Why here and not at submit: finishMock redirects straight to this page, so the
 * player never gets a chance to trigger anything — and scoring inline would hold
 * the submit open for several seconds per answer. Both actions are idempotent
 * (they only touch rows with no band), so a refresh or a second visit can't
 * double-score or re-charge the API.
 */
export function SpeakingScoreTrigger({ sessionId }: { sessionId: string }) {
  const router = useRouter();
  const [state, setState] = useState<"scoring" | "done" | "failed">("scoring");
  // React runs effects twice in dev StrictMode; without this the action fires
  // twice on every mount.
  const fired = useRef(false);

  useEffect(() => {
    if (fired.current) return;
    fired.current = true;

    Promise.allSettled([scoreMockWriting(sessionId), scoreMockSpeaking(sessionId)])
      .then((results) => {
        setState("done");
        const scored = results.some((r) => r.status === "fulfilled" && r.value.scored > 0);
        // Only re-render the report if a band actually landed.
        if (scored) router.refresh();
      })
      .catch(() => setState("failed"));
  }, [sessionId, router]);

  if (state === "done") return null;

  return (
    <p
      className={`flex items-center gap-2 rounded-lg px-3 py-2 text-xs ${
        state === "failed" ? "bg-danger-soft text-danger" : "bg-info-soft text-ink-soft"
      }`}
    >
      {state === "scoring" ? (
        <>
          <Loader2 className="size-3.5 animate-spin" />
          Scoring your writing &amp; speaking answers with AI — this takes a few seconds each. The
          report will update automatically.
        </>
      ) : (
        <>
          <Sparkles className="size-3.5" />
          Some answers couldn&apos;t be scored right now. Your answers are saved — reload to retry.
        </>
      )}
    </p>
  );
}
