...
import { planAtLeast, type PlanKey } from "@/lib/plans";
...
export function PlanCta({
  ...
  if (authenticated && current) {
    ...
    // A real step up. There is no gateway yet, so the way to take it is to ask.
    return (
      <Link href="/contact" className={cn(base, featured ? primary : secondary)}>
        Contact us to upgrade <ArrowRight className="size-4" />
      </Link>
    );
  }
