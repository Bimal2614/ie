...
export function MatchingBoard({
  ...
  return (
    /**
     * CONTAINER query, not a viewport breakpoint. This board is nearly always
     * drawn inside the questions pane of a resizable split, which stays narrow
     * even on a wide monitor. `lg:` measured the WINDOW, so a 450px pane on a
     * 1080p screen was still cut into two columns: the stem column collapsed to
     * one word per line and unbreakable words ran under the drop box. Asking the
     * PANE how wide it is keeps the stems above and the bank below until the
     * split genuinely fits — which is the order a candidate wants anyway.
     */
    <div className="@container">
      <div className="grid gap-4 @2xl:grid-cols-[minmax(0,1fr)_minmax(0,16rem)] @2xl:items-start">
        {/* ── Stems: the numbered prompts, each with a blank ── */}
          ...
          {items.map((item) => {
            ...
            return (
              <li
                key={item.n}
                id={`sq-${item.n}`}
                // `min-w-36` is what makes the wrap fire. A bare `flex-1` stem shrinks to
                // zero before the row ever wraps, so the prompt was squeezed to a
                // one-character column while the 11rem box kept its full width.
                className="flex scroll-mt-28 flex-wrap items-center gap-x-3 gap-y-2"
              >
                <span className="min-w-36 flex-1 break-words text-sm text-ink-soft">
        ...
        {/* ── The option bank — beside the stems with room, under them without ── */}
